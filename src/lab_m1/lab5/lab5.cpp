...
void Lab5::Init() {
	...
	camera->Set(glm::vec3(0, 1, -1), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0), 3);
	...
	projectionMatrix = glm::perspective(projAngle, window->props.aspectRatio, 0.01f, 200.0f);
}
...
void Lab5::Update(float deltaTimeSeconds) {
...

	{
		...
		modelMatrix *= glm::rotate(modelMatrix, TO_RADIANS * 30, glm::vec3(1, 0, 0));
		RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	}
...
void Lab5::OnInputUpdate(float deltaTime, int mods) {
	...
	if (window->KeyHold(GLFW_KEY_P)) {
		if (window->KeyHold(GLFW_KEY_X)) {
			projAngle += deltaTime * TO_RADIANS * 30;
			projectionMatrix = glm::perspective(projAngle, window->props.aspectRatio, 0.01f, 200.0f);
		}
	...
	if (window->KeyHold(GLFW_KEY_O)) {
		if (window->KeyHold(GLFW_KEY_X)) {
			...
			right = window->props.aspectRatio * top;
			left = -right;
			bottom = -top;
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY) {
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) {
		...
		float angleOY = -float(deltaX) * TO_RADIANS * sensivityOX;
		float angleOX = -float(deltaY) * TO_RADIANS * sensivityOY;
