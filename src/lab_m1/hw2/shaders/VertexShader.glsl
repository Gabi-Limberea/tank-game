...
void main()
{
    ...
    diffuse_light = object_color  * material_kd * max(dot(N, L), 0.0);
    ...
    if (diffuse_light.x > 0 || diffuse_light.y > 0 || diffuse_light.z > 0)
    {
        ...
        // specular_light =  object_color * material_ks * pow(max(dot(N, H), 0.0), material_shininess);
    }
    ...
    if (health_in != -1.0) {
        new_position = new_position + (health_in - 100.0f)  * 0.075f * cnoise(v_position * 0.1f);
    }
